...
class LinfPGDAttack(object):
    def __init__(self, model=None, device=None, epsilon=0.05, k=10, a=0.01, feat=None, adv_model=None, fft_mode=None,
                 fft_center=30, fft_protect=False):
        """
    ...
    def get_amp_ang(self, X):
        ...
        _ang.append((torch.DoubleTensor(torch.from_numpy(ang[0]))).cuda())
        _ang.append((torch.DoubleTensor(torch.from_numpy(ang[1]))).cuda())
        _ang.append((torch.DoubleTensor(torch.from_numpy(ang[2]))).cuda())

        _amp = torch.stack((_amp[0], _amp[1], _amp[2]), 0)
        _amp = torch.unsqueeze(_amp, 0)
        _amp = _amp.to(torch.float64)

        _ang = torch.stack((_ang[0], _ang[1], _ang[2]), 0)
        _ang = torch.unsqueeze(_ang, 0)
        _ang = _ang.to(torch.float64)
    ...
    def protect(self, now, backup, _range):
        ...
        for i in range(3):
            now[0][i][row - _range:row + _range - 1, col - _range:col + _range - 1] = backup[0][i][
                                                                                      row - _range:row + _range - 1,
    ...
    def recover(self, amp, ang, backup):
        ...
        return: (3,256,256)tensor
        '''
        if self.fft_protect:
            if self.fft_mode == 'amp':
                amp = self.protect(amp, backup, self.fft_center // 2)

            elif self.fft_mode == 'ang':
                ang = self.protect(ang, backup, self.fft_center // 2)

        _re = []
        for i in range(3):
            re = (amp[0][i].cpu().numpy() * np.cos(ang[0][i].cpu().numpy()) + (amp[0][i].cpu().numpy()) * np.sin(
                ang[0][i].cpu().numpy()) * 1j)
    ...
    def perturb(self, X_nat, y, c_trg):
        ...
        elif self.fft_mode != None:
            if self.rand:
                X = X_nat.clone().detach_()
                X_amp, X_ang = self.get_amp_ang(X)
                random_noise = torch.tensor(
                    np.random.uniform(-self.epsilon, self.epsilon, X_nat.shape).astype('float32')).to(self.device)
                random_noise_amp, random_noise_ang = self.get_amp_ang(random_noise)

                if self.fft_mode == 'amp':
                    X_amp += random_noise_amp
                # elif self.fft_mode == 'ang':
                #     X_ang += random_noise_ang
        ...
        for i in range(self.k):
            ...
            loss = self.loss_fn(Y.float(), output.float())
            ...
            eta = torch.clamp(X_adv - X_backup, min=-self.epsilon, max=self.epsilon)
            X = torch.clamp(X_backup + eta, min=-1, max=1).detach_()
