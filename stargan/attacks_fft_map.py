...
class LinfPGDAttack(object):
    ...
    def reverse(self, cmap, ang):
        _re = []
        for i in range(3):
            re = (cmap[i].cpu().numpy() * np.cos(ang[i]) + cmap[i].cpu().numpy() * np.sin(ang[i]) * 1j)
    ...
    def perturb(self, X_nat, y, c_trg):
        ...
        for i in range(self.k):
            ...
            X_map[0] = (torch.DoubleTensor(torch.from_numpy(X_map[0]))).cuda()
            X_map[1] = (torch.DoubleTensor(torch.from_numpy(X_map[1]))).cuda()
            X_map[2] = (torch.DoubleTensor(torch.from_numpy(X_map[2]))).cuda()

            X = torch.stack((X_map[0], X_map[1], X_map[2]), 0)
            ...
            Y_map[0] = (torch.DoubleTensor(torch.from_numpy(Y_map[0]))).cuda()
            Y_map[1] = (torch.DoubleTensor(torch.from_numpy(Y_map[1]))).cuda()
            Y_map[2] = (torch.DoubleTensor(torch.from_numpy(Y_map[2]))).cuda()

            Y = torch.stack((Y_map[0], Y_map[1], Y_map[2]), 0)
        ...
        X_nat_map[0] = (torch.DoubleTensor(torch.from_numpy(X_nat_map[0]))).cuda()
        X_nat_map[1] = (torch.DoubleTensor(torch.from_numpy(X_nat_map[1]))).cuda()
        X_nat_map[2] = (torch.DoubleTensor(torch.from_numpy(X_nat_map[2]))).cuda()

        X_nat = torch.stack((X_nat_map[0], X_nat_map[1], X_nat_map[2]), 0)
        ...
        per = self.reverse(X - X_nat, X_ang)
        X = self.reverse(X, X_ang)
